 ...
 * 
 * Intuition
The problem aims to find the maximum profit that can be obtained by buying and selling a stock. 
The given solution seems to follow a simple approach of iterating through the prices, 
keeping track of the minimum buying price, and updating the profit whenever a higher selling price is encountered.
...
Complexity
Time complexity: O(n)O(n)O(n), where nnn is the length of the prices array. The algorithm iterates through the array once.
Space complexity: O(1)O(1)O(1), as only a constant amount of extra space is used.
 * 
*/

package stringManipulation;

public class BestTimeToBuyAStock_150{
